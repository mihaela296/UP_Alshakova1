...
namespace UP_Alshakova
{
    public partial class ProductsWindow : Window
    {
        ...
        public ProductsWindow(string userRole, string userName)
        {
            InitializeComponent();
            _userRole = userRole;
            _userName = userName;
        ...
        private void InitializePermissions()
        {
            ...
            string roleText = _userRole ?? "Гость";
            txtWindowTitle.Text = $"Товары - {roleText}";

            // Настраиваем видимость элементов в зависимости от роли
            if (_userRole == null) // Гость
            {
            ...
            else if (_userRole == "Client") // Клиент
            {
                pnlSearch.Visibility = Visibility.Collapsed;
                btnOrders.Visibility = Visibility.Collapsed; // или Visible если клиент может видеть свои заказы
                btnAddProduct.Visibility = Visibility.Collapsed;
                colActions.Visibility = Visibility.Collapsed;
            }
            else if (_userRole == "Manager") // Менеджер
            {
            ...
            else if (_userRole == "Admin") // Администратор
            {
                pnlSearch.Visibility = Visibility.Visible;
                btnOrders.Visibility = Visibility.Visible;
        ...
        {
            OrdersWindow ordersWindow = new OrdersWindow(_userRole, _userName);
            ordersWindow.Show();
            this.Close();
        }
        ...
        {
            if (_userRole == "Admin" && dgProducts.SelectedItem != null)
            {
                dynamic selectedProduct = dgProducts.SelectedItem;
                // Открываем окно редактирования для администратора
        ...
        private void mnuViewDetails_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                dynamic selectedProduct = dgProducts.SelectedItem;
                ProductDetailsWindow detailsWindow = new ProductDetailsWindow(selectedProduct.ProductID);
                detailsWindow.ShowDialog();
            }
        }

        private void mnuEditProduct_Click(object sender, RoutedEventArgs e)
        {
            if (_userRole == "Admin" && dgProducts.SelectedItem != null)
            {
                dynamic selectedProduct = dgProducts.SelectedItem;
